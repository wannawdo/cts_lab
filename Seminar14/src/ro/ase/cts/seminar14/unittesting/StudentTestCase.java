...
class StudentTestCase {
	...
	@Test
	void testCalculMedieCardinalityZero() throws StudentExceptionWrongValue {
		int note[]=new int[3];
		try {
		student.setNote(note);
		}
		...
		assertThrows(StudentExceptionWrongValue.class,()->student.calculMedie());
	}

	//cardinality test - 1 element
	@Test
	void testCalculMedieCardinalityOne() throws StudentExceptionWrongValue {
		int note[]=new int[1];
		note[0]=9;
		try {
		student.setNote(note);
